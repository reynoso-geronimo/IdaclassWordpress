...
// Output lable
echo '<div class="yayextra-option-field-wrap" data-option-field-id="' . esc_attr( $data['id'] ) . '" data-option-field-type="date_picker">';
Utils::get_template_part( $template_folder, 'label_field', array( 'data' => $data ) );
echo '<div ><input type="text" id="yayextra-date-picker' . esc_attr( $data['id'] ) . '" class="yayextra-date-picker ' . esc_attr( $class_names ) . '" name="option_field_data[' . esc_attr( $opt_set_id ) . '][' . esc_attr( $data['id'] ) . ']" value="' . esc_attr( $value ) . '"/></div>';
echo '</div>';
