...
namespace YayExtra\YayCommerceMenu;
...
class RegisterMenu {
	...
	public function __construct() {
		if ( ! defined( 'YAYE_MENU_ORDER' ) ) {
			define( 'YAYE_MENU_ORDER', 4 );
		}
		if ( ! defined( 'YAYE_MENU_PRIORITY' ) ) {
			define( 'YAYE_MENU_PRIORITY', 70 );
		}
		add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_yaycommerce_menu_scripts' ) );
		add_action( 'admin_menu', array( $this, 'settings_menu' ) );
		add_action( 'admin_menu', array( $this, 'add_placeholder_menu' ), YAYE_MENU_PRIORITY + 1 );
	...
	public function add_placeholder_menu() {
		...
		if ( ! $has_plugin_menu ) {
			add_submenu_page( 'yaycommerce', __( 'YayExtra', 'yayextra' ), __( 'YayExtra', 'yayextra' ), 'manage_woocommerce', 'yayextra', array( $this, 'render_placeholder_menu' ), 0 );
		}
	}
